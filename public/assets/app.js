...
qsa('.sidebar-link', sidebar).forEach(link => {
...
function dashboardView() {
  ...
  if (hour < 5) greeting = 'Selamat malam';
  else if (hour < 12) greeting = 'Selamat pagi';
  else if (hour < 15) greeting = 'Selamat siang';
  else if (hour < 19) greeting = 'Selamat sore';
  else greeting = 'Selamat malam';

  const greetEl = el('div', { class: 'dashboard-greeting' }, [
    document.createTextNode(greeting + ', '),
    el('span', { html: 'fbyanxzz' }),
    document.createTextNode(' 👋')
  ]);

  const descEl = el('p', { class: 'subtitle' }, [
    document.createTextNode('Yuk download video atau audio dari berbagai platform di sini. Cukup paste link, proses, lalu download.')
  ]);
  ...
  const stats = el('div', { class: 'dashboard-stats' }, [
    ...
    statCard('10+', 'Tools'),
    statCard('24/7', 'Akses'),
    statCard('Gratis', 'Tanpa Daftar')
  ]);

  function statCard(num, label) {
    return el('div', { class: 'stat-card' }, [
      el('div', { class: 'stat-number', html: num }),
  ...
  const tools = el('div', { class: 'tool-grid' }, [
    toolCard('TikTok', 'fab fa-tiktok', '/tiktok'),
    toolCard('Instagram', 'fab fa-instagram', '/instagram'),
    toolCard('YouTube', 'fab fa-youtube', '/youtube'),
    toolCard('Facebook', 'fab fa-facebook', '/facebook'),
    toolCard('Spotify', 'fab fa-spotify', '/spotify'),
    toolCard('QR Generator', 'fas fa-qrcode', '/qr-generator')
  ]);

  function toolCard(label, icon, path) {
    return el('a', { class: 'tool-card', href: path, 'data-link': '' }, [
      el('i', { class: icon }),
      el('span', { html: label })
    ]);
  }
  ...
  return el('div', { class: 'panel' }, [
    ...
    descEl,
    stats,
    el('div', { class: 'title', style: 'margin-top:8px; font-size:18px' }, [
      el('i', { class: 'fas fa-th-large' }),
